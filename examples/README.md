...
| Makefile switch | CMake switch | Description | Additional information |
| --- | --- | --- | --- |
| BACKBONE_ROUTER | OT_BACKBONE_ROUTER | Enable backbone router functionality. |
...
| LINK_RAW | OT_LINK_RAW | Enable Link Raw service. | |
...
| UDP_FORWARD | OT_UDP_FORWARD | Enable UDP forward support. | Enable this switch if handling UDP packets on NCP from host is required (on Border Router device). |
