...
Commissioner role is supported.
   - Use cases:
Enable this switch on device that should be able to perform Commissioner role.
...
https://openthread.io/guides/build/features/child-supervision
   - Typical use case:
Enable this switch on parent or child node with custom OpenThread application that manages the supervision, checks timeout intervals and verifies connectivity between parent and child.
...
The device is able to act as typical DHCP client.
   - Typical use case:
Enable this switch on device that is supposed to request the DHCP server for networking parameters.

- DHCP6_SERVER:
   - If enabled:
The device is able to act as typical DHCP server.
   - Typical use case:
Enable this switch on device that is supposed to provide networking parameters to devices with DHCP_CLIENT switch enabled.

- DIAGNOSTIC:
   - If enabled:
Functions in platform abstraction for diagnostics feature module are enabled.
   - Typical use case:
Enable this switch on device that is tested in the factory production stage.
...
Functions in DNS feature module are enabled.
   - Typical use case:
Enable this switch on device that sends a DNS query for AAAA (IPv6) record.
...
https://openthread.io/reference/group/api-logging
   - Typical use case:
Enable this switch if OpenThread log level is required to be set in runtime.

- ECDSA:
   - If enabled:
The support for ECDSA is enabled in crypto module.
   - Typical use case:
Enable this switch if ECDSA digital signature is used by application.

- EXTERNAL_HEAP:
   - If enabled:
The external heap Calloc and Free functions to be used by the OpenThread stack must be specified.
   - Typical use case:
Enable this switch if the platform utilizes its own heap.
...
https://openthread.io/guides/build/features/jam-detection
   - Typical use case:
Enable this switch if device requires the ability to detect signal jamming on a specific channel.
...
https://openthread.io/reference/group/api-joiner
   - Typical use case:
Enable this switch on device that has to be commissioned to join the network.
